...
class Intersection:
    def __init__(self, inter_id, dic_traffic_env_conf, eng, light_id_dict, path_to_log, lanes_length_dict):
        ...
        self.adjacency_row_2hop = light_id_dict.get("adjacency_row_2hop", self.adjacency_row)  # 二阶邻居
        self.neighbor_ENWS = light_id_dict["neighbor_ENWS"]
        self.neighbor_to_index = light_id_dict.get("neighbor_to_index", {})  # 邻居方向到index的映射
    ...
    def _update_feature(self):
        ...
        # 根据配置选择邻接矩阵类型
        if self.dic_traffic_env_conf.get("USE_2HOP_NEIGHBOR", False):
            dic_feature["adjacency_matrix"] = self._get_adjacency_row_2hop()
        else:
            dic_feature["adjacency_matrix"] = self._get_adjacency_row()
        
        # 计算动态邻接权重（如果启用）
        if self.dic_traffic_env_conf.get("USE_DYNAMIC_ADJACENCY", False):
            dic_feature["adjacency_weights"] = self._compute_dynamic_adjacency_weights(
    ...
    def _get_adjacency_row_2hop(self):
        """返回包含二阶邻居的邻接行"""
        return self.adjacency_row_2hop
    
    def _compute_dynamic_adjacency_weights(self, lane_num_vehicle):
        """
        计算基于车流量的动态邻接权重
        weight[j] = 邻居j方向的入流车辆数 / 总入流车辆数
        """
        # 计算各方向的车辆数：W(0-2), E(3-5), N(6-8), S(9-11)
        direction_vehicles = {
            "W": sum(lane_num_vehicle[0:3]),
            "E": sum(lane_num_vehicle[3:6]),
            "N": sum(lane_num_vehicle[6:9]),
            "S": sum(lane_num_vehicle[9:12])
        }
        total_vehicles = sum(direction_vehicles.values())
        
        # 计算每个邻居的权重
        weights = []
        decay = self.dic_traffic_env_conf.get("NEIGHBOR_DECAY_FACTOR", 0.5)
        
        for idx, neighbor in enumerate(self.adjacency_row):
            if idx == 0:  # 自己
                weights.append(1.0)
            else:
                # 获取邻居方向
                direction = self.neighbor_ENWS[idx - 1] if idx - 1 < len(self.neighbor_ENWS) else None
                if direction and direction in ["E", "W", "N", "S"]:
                    # 根据入流方向计算权重
                    dir_map = {"E": "W", "W": "E", "N": "S", "S": "N"}  # 入流方向映射
                    in_dir = dir_map.get(direction, direction)
                    if total_vehicles > 0:
                        weight = direction_vehicles.get(in_dir, 0) / total_vehicles
                    else:
                        weight = 0.25  # 默认均匀分布
                    weights.append(weight)
                else:
                    weights.append(decay)  # 二阶邻居使用衰减因子
...
class CityFlowEnv:
    ...
    def _adjacency_extraction(self):
        ...
        with open("{0}".format(file)) as json_data:
            ...
            # ===== 计算二阶邻居 (2-hop neighbors) =====
            if self.dic_traffic_env_conf.get("USE_2HOP_NEIGHBOR", False):
                for i in traffic_light_node_dict.keys():
                    one_hop = traffic_light_node_dict[i]["adjacency_row"]
                    two_hop_set = set(one_hop)  # 从一阶邻居开始
                    
                    # 遍历每个一阶邻居，获取它们的邻居
                    for neighbor_idx in one_hop[1:]:  # 跳过自己 (index 0)
                        # 找到对应的 intersection id
                        for inter_id, idx in inter_id_to_index.items():
                            if idx == neighbor_idx:
                                neighbor_one_hop = traffic_light_node_dict[inter_id]["adjacency_row"]
                                two_hop_set.update(neighbor_one_hop)
                                break
                    
                    # 构建二阶邻接行：自己 + 一阶邻居 + 二阶邻居（去重）
                    two_hop_list = list(two_hop_set)
                    # 确保自己在第一位
                    if inter_id_to_index[i] in two_hop_list:
                        two_hop_list.remove(inter_id_to_index[i])
                    two_hop_list = [inter_id_to_index[i]] + two_hop_list[:top_k-1]
                    
                    traffic_light_node_dict[i]["adjacency_row_2hop"] = two_hop_list
            else:
                # 不使用二阶邻居时，复制一阶邻居
                for i in traffic_light_node_dict.keys():
                    traffic_light_node_dict[i]["adjacency_row_2hop"] = traffic_light_node_dict[i]["adjacency_row"]
