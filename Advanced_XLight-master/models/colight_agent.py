...
class CoLightAgent(Agent):
    def __init__(self, dic_agent_conf=None, dic_traffic_env_conf=None, dic_path=None, cnt_round=None,
        ...
        if cnt_round == 0:
            ...
            # 尝试加载预训练权重（如果存在）
            pretrain_path = os.path.join(self.dic_path["PATH_TO_MODEL"], "round_0_inter_{0}".format(intersection_id))
            pretrain_h5 = pretrain_path + ".h5"
            if os.path.exists(pretrain_path) and os.path.isdir(pretrain_path):
                # SavedModel 格式
                self.q_network = tf.keras.models.load_model(pretrain_path)
            elif os.path.exists(pretrain_h5):
                # 旧版 h5 格式
                self.q_network.load_weights(pretrain_h5, by_name=True)
            # 如果都不存在，则使用新初始化的网络
    ...
    def load_network(self, file_name, file_path=None):
        ...
        # 尝试加载 SavedModel 格式
        saved_model_path = os.path.join(file_path, file_name)
        old_h5_file = os.path.join(file_path, "%s.h5" % file_name)
        if os.path.exists(saved_model_path) and os.path.isdir(saved_model_path):
            self.q_network = tf.keras.models.load_model(saved_model_path)
            print("succeed in loading SavedModel %s" % file_name)
        elif os.path.exists(old_h5_file):
            self.q_network = load_model(old_h5_file, custom_objects={'RepeatVector3D': RepeatVector3D})
            print("succeed in loading h5 model %s" % file_name)
        else:
            print("Warning: model file not found for %s, building new network" % file_name)
            self.q_network = self.build_network()
        # 确保 q_network_bar 已初始化
        if not hasattr(self, 'q_network_bar') or self.q_network_bar is None:
            self.q_network_bar = self.build_network_from_copy(self.q_network)

    def load_network_bar(self, file_name, file_path=None):
        if file_path is None:
            file_path = self.dic_path["PATH_TO_MODEL"]
        saved_model_path = os.path.join(file_path, file_name + "_bar")
        old_h5_file = os.path.join(file_path, "%s.h5" % file_name)
        if os.path.exists(saved_model_path) and os.path.isdir(saved_model_path):
            self.q_network_bar = tf.keras.models.load_model(saved_model_path)
            print("succeed in loading SavedModel %s_bar" % file_name)
        elif os.path.exists(old_h5_file):
            self.q_network_bar = load_model(old_h5_file, custom_objects={'RepeatVector3D': RepeatVector3D})
            print("succeed in loading h5 model %s" % file_name)
        else:
            # 如果找不到 bar 模型，从 q_network 复制
            print("Warning: bar model not found for %s, copying from q_network" % file_name)
            self.q_network_bar = self.build_network_from_copy(self.q_network)

    def save_network(self, file_name):
        # 使用 TensorFlow SavedModel 格式，绕过 h5py 兼容性问题
        save_path = os.path.join(self.dic_path["PATH_TO_MODEL"], file_name)
        self.q_network.save(save_path, save_format='tf')

    def save_network_bar(self, file_name):
        save_path = os.path.join(self.dic_path["PATH_TO_MODEL"], file_name + "_bar")
        self.q_network_bar.save(save_path, save_format='tf')
