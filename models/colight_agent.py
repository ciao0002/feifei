...
class CoLightAgent(Agent):
    def __init__(self, dic_agent_conf=None, dic_traffic_env_conf=None, dic_path=None, cnt_round=None,
        ...
        if cnt_round == 0:
            ...
            if os.listdir(self.dic_path["PATH_TO_MODEL"]):
                # Use standard load_network which calls load_model internally
    ...
    def train_network(self):
        if not hasattr(self, 'Y') or len(self.Y) == 0:
            return
    ...
    def load_network(self, file_name, file_path=None):
        ...
        # Removed .h5 for SavedModel format compatibility
        self.q_network = load_model(
            os.path.join(file_path, "%s" % file_name),
    ...
    def save_network(self, file_name):
        # Save as SavedModel (directory) instead of .h5 to avoid h5py precision errors
        self.q_network.save(os.path.join(self.dic_path["PATH_TO_MODEL"], "%s" % file_name), save_format='tf')

    def save_network_bar(self, file_name):
        # Save as SavedModel (directory) instead of .h5 to avoid h5py precision errors
        self.q_network_bar.save(os.path.join(self.dic_path["PATH_TO_MODEL"], "%s" % file_name), save_format='tf')
