...
def path_check(dic_path, resume=False):
    if os.path.exists(dic_path["PATH_TO_WORK_DIRECTORY"]):
        if dic_path["PATH_TO_WORK_DIRECTORY"] != "records/default" and not resume:
            raise FileExistsError
    else:
        os.makedirs(dic_path["PATH_TO_WORK_DIRECTORY"])
    if os.path.exists(dic_path["PATH_TO_MODEL"]):
        if dic_path["PATH_TO_MODEL"] != "model/default" and not resume:
            raise FileExistsError
    else:
...
class Pipeline:
    ...
    def initialize(self):
        path_check(self.dic_path, resume=self.dic_traffic_env_conf.get("RESUME", False))
        # Only copy config if not resuming, or overwrite if desired
    ...
    def run(self, multi_process=False):
        ...
        for cnt_round in range(self.dic_traffic_env_conf["NUM_ROUNDS"]):
            if self.dic_traffic_env_conf.get("RESUME", False):
                ...
                round_dir = os.path.join(self.dic_path["PATH_TO_WORK_DIRECTORY"], "test_round", f"round_{cnt_round}")
                if os.path.exists(round_dir):
                    print(f"Skipping completed round {cnt_round}")
                    continue
