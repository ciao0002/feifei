...
class Intersection:
    def __init__(self, inter_id, dic_traffic_env_conf, eng, light_id_dict, path_to_log, lanes_length_dict):
        ...
        with open(path_to_log_file, "a") as f:
            f.write("{0},{1}\n".format(float(self.get_current_time()), int(self.current_phase_index)))
    ...
    def _update_arrive_time(self, list_vehicle_arrive):
        # Force float conversion to avoid TypeError: Cannot convert numpy.ndarray to numpy.ndarray
        ts = float(self.get_current_time())
    ...
    def _update_left_time(self, list_vehicle_left):
        # Force float conversion to avoid TypeError
        ts = float(self.get_current_time())
    ...
    def _update_feature(self):
        # Only calculate features that are actually needed for LIST_STATE_FEATURE or rewards
        used_features = set(self.dic_traffic_env_conf.get("LIST_STATE_FEATURE", []))
        used_features.add("cur_phase")
        
        # Check reward info for dependencies
        reward_info = self.dic_traffic_env_conf.get("DIC_REWARD_INFO", {})
        if reward_info.get("pressure", 0) != 0:
            used_features.add("pressure")
        if reward_info.get("queue_length", 0) != 0:
            used_features.add("lane_num_waiting_vehicle_in")
        
        # Always add base features if we need pressure
        if "pressure" in used_features:
            used_features.add("lane_num_waiting_vehicle_in")
            used_features.add("lane_num_waiting_vehicle_out")
        ...
        if "lane_num_vehicle_downstream" in used_features or "traffic_movement_pressure_num" in used_features:
            dic_feature["lane_num_vehicle_downstream"] = self._get_lane_num_vehicle_downstream()
            
        if "lane_num_waiting_vehicle_in" in used_features:
        ...
        if "traffic_movement_pressure_queue_efficient" in used_features:
            # Re-ensure base features for this advanced feature if they were missed
            in_q = dic_feature.get("lane_num_waiting_vehicle_in") or self._get_lane_queue_length(self.list_entering_lanes)
            out_q = dic_feature.get("lane_num_waiting_vehicle_out") or self._get_lane_queue_length(self.list_exiting_lanes)
            dic_feature["traffic_movement_pressure_queue_efficient"] = self._get_traffic_movement_pressure_efficient(in_q, out_q)

        if "lane_enter_running_part" in used_features:
            # This triggers the expensive _get_part_observations
            _, _, _, enter_running_part, _ = self._get_part_traffic_movement_features()
            dic_feature["lane_enter_running_part"] = list(enter_running_part)

        if "lane_enter_num_vehicle_front" in used_features:
            # Vehicles near stop line within OBS_LENGTH (includes queue vehicles).
            dic_feature["lane_enter_num_vehicle_front"] = self._get_lane_enter_num_vehicle_front()

        if "pressure" in used_features:
            dic_feature["pressure"] = self._get_pressure()
            
        if "adjacency_matrix" in used_features:
            dic_feature["adjacency_matrix"] = self._get_adjacency_row()

        # Extremely heavy feature - CRITICAL to skip if not in use
        if "num_in_seg_attend" in used_features:
            dic_feature["num_in_seg_attend"] = self._orgnize_several_segments_attend(
                dic_feature.get("lane_num_waiting_vehicle_in", self._get_lane_queue_length(self.list_entering_lanes)),
                dic_feature.get("lane_num_waiting_vehicle_out", self._get_lane_queue_length(self.list_exiting_lanes)))
    ...
    def _get_several_segments_attend(self, lane_vehicles, vehicle_distance, vehicle_speed,
        ...
        obs_length = 100
        part1, part2, part3 = {lane: [] for lane in list_lanes}, {lane: [] for lane in list_lanes}, {lane: [] for lane in list_lanes}
        
        for lane in list_lanes:
            current_lane_vehicles = lane_vehicles[lane]
            if not current_lane_vehicles:
                continue
            
            l_len = lane_length[lane]
            for vehicle in current_lane_vehicles:
                if "shadow" in vehicle:
                    continue
                
                v_speed = vehicle_speed.get(vehicle, 0)
                if v_speed > 0.1:
                    dist = vehicle_distance.get(vehicle, 0)
                    if dist > l_len - obs_length:
                        part1[lane].append(vehicle)
                    elif l_len - 2 * obs_length < dist <= l_len - obs_length:
                        part2[lane].append(vehicle)
                    elif l_len - 3 * obs_length < dist <= l_len - 2 * obs_length:
    ...
    def _get_lane_enter_num_vehicle_front(self):
        """
        Vehicles in front (effective distance approximation):
        Count vehicles in the last OBS_LENGTH meters of each entering lane.
        This corresponds to e_i in HolLight ([e_i, n_i, p_i]) and includes queued vehicles.
        """
        _, last_part_num_vehicle, _ = self._get_part_observations(
            lane_vehicles=self.dic_lane_vehicle_current_step,
            vehicle_distance=self.dic_vehicle_distance_current_step,
            vehicle_speed=self.dic_vehicle_speed_current_step,
            lane_length=self.lane_length,
            obs_length=self.obs_length,
            list_lanes=self.list_entering_lanes,
        )
        return [len(last_part_num_vehicle[lane]) for lane in self.list_entering_lanes]

    @staticmethod
    def _get_part_observations(lane_vehicles, vehicle_distance, vehicle_speed,
                               lane_length, obs_length, list_lanes):
        first_part_num_vehicle = {lane: [] for lane in list_lanes}
        last_part_num_vehicle = {lane: [] for lane in list_lanes}
        last_part_queue_vehicle = {lane: [] for lane in list_lanes}
        ...
        for lane in list_lanes:
            ...
            l_len = lane_length[lane]
            last_part_threshold = l_len - obs_length
...
class CityFlowEnv:
    ...
    def reset(self):
        ...
        cityflow_config = {
            ...
            "seed": self.dic_traffic_env_conf.get("seed", 0),
            "laneChange": True,
    ...
    def get_reward(self):
        ...
        neighbor_coef = float(reward_info.get("neighbor_queue_length", 0.0))

        # Default path: legacy local reward only.
        if neighbor_coef == 0.0:
            return [inter.get_reward(reward_info) for inter in self.list_intersection]

        local_reward = []
        local_queue = []
        for inter in self.list_intersection:
            local_reward.append(inter.get_reward(reward_info))
            lane_wait = inter.dic_feature.get("lane_num_waiting_vehicle_in", [])
            local_queue.append(float(np.absolute(np.sum(lane_wait))))

        list_reward = []
        for idx, inter in enumerate(self.list_intersection):
            neighbor_q = []
            for neighbor_id in inter.neighbor_ENWS:
                if neighbor_id is None:
                    continue
                neighbor_idx = self.id_to_index.get(neighbor_id)
                if neighbor_idx is None:
                    continue
                neighbor_q.append(local_queue[neighbor_idx])
            neighbor_mean = float(np.mean(neighbor_q)) if neighbor_q else 0.0
            list_reward.append(local_reward[idx] + neighbor_coef * neighbor_mean)
        return list_reward
    ...
    def log(self, cur_time, before_action_feature, action):
        if not self.dic_traffic_env_conf.get("saveReplay", False): return
    ...
    def batch_log_2(self):
        ...
        for inter_ind in range(self.dic_traffic_env_conf["NUM_INTERSECTIONS"]):
            ...
            # Note: batch_log_2 doesn't usually write pkl files in this version
            test_env = self.eng

    def batch_log(self, start, stop):
        # 1. First write all CSVs (critical for metrics)
        ...
        # 2. Then attempt pickle (non-critical for PPO, can fail)
        for inter_ind in range(start, stop):
            try:
                path_to_log_file = os.path.join(self.path_to_log, "inter_{0}.pkl".format(inter_ind))
                with open(path_to_log_file, "wb") as f:
                    # Defensive copy/clean to help pickle
                    pickle.dump(self.list_inter_log[inter_ind], f)
            except Exception as e:
                print(f"Warning: Failed to pickle log for inter {inter_ind}: {e}")

    def bulk_log_multi_process(self, batch_size=100):
        assert len(self.list_intersection) == len(self.list_inter_log)
        # Use single-process sequential logging because Intersection objects 
        # cannot be reliably pickled for multiprocessing in this environment.
        # This ensures ALL intersections (e.g., 0-15) are logged to CSV.
        print(f"Logging {len(self.list_intersection)} intersections sequentially...")
        self.batch_log(0, len(self.list_intersection))
