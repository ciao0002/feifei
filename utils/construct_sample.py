...
def get_reward_from_features(rs):
    lane_wait = rs.get("lane_num_waiting_vehicle_in", 0)
    if lane_wait is None:
        lane_wait = 0
    pressure_val = rs.get("pressure", 0)
    if pressure_val is None:
        pressure_val = 0
    reward = {
        "queue_length": np.sum(lane_wait),
        "pressure": np.absolute(np.sum(pressure_val)),
    }
    return reward
        ...
        if self.dic_traffic_env_conf["BINARY_PHASE_EXPANSION"]:
            ...
            for key, value in state["state"].items():
                if key in features:
                    if "cur_phase" in key:
                        phase_key = value[0]
                        # Some configs store PHASE keys as strings in JSON; normalize here.
                        if phase_key not in self.dic_traffic_env_conf['PHASE']:
                            phase_key = str(phase_key)
                        state_after_selection[key] = self.dic_traffic_env_conf['PHASE'][phase_key]
                    else:
    ...
    def dump_sample(self, samples, folder):
        if folder == "":
            ...
            with open(os.path.join(self.parent_dir, "total_samples_{0}.pkl".format(folder)), "ab+") as f:
                pickle.dump(samples if samples is not None else [], f, -1)
        else:
