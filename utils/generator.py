...
class Generator:
    def __init__(self, cnt_round, cnt_gen, dic_path, dic_agent_conf, dic_traffic_env_conf):
        ...
        # Dynamic seed update: base_seed + round_number
        # This ensures that even if the base seed is fixed, each round gets a different seed
        if "seed" in self.dic_traffic_env_conf:
            self.dic_traffic_env_conf["seed"] = int(self.dic_traffic_env_conf["seed"]) + int(cnt_round)
